...
const NEW = `async function eliminarTorneo(tournament) {
  ...
  const confirmed = await new Promise(function(resolve) {
    ...
    overlay.innerHTML = '<div style="background:#0f1b2d;border:1px solid rgba(56,189,248,0.2);border-radius:16px;padding:24px;max-width:420px;width:90%;box-shadow:0 8px 32px rgba(0,0,0,0.5)">' +
      ...
      '<span style="color:#f87171;font-weight:700">Esta acción NO se puede deshacer.</span></div>' +
    ...
    document.getElementById('cancelDelete').onclick = function() {
      var el = document.getElementById('deleteTournamentOverlay');
      if (el) el.remove();
      resolve(false);
    };
  ...
  var confirmText = await new Promise(function(resolve) {
    ...
    document.getElementById('cancelInput').onclick = function() {
      var el = document.getElementById('confirmTournamentOverlay');
      if (el) el.remove();
      resolve(null);
    };
    document.getElementById('confirmInput').onclick = function() {
      var v = inp ? inp.value : '';
      var el = document.getElementById('confirmTournamentOverlay');
      if (el) el.remove();
      resolve(v);
  ...
  for (var i = 0; i < matches.length; i++) {
    await DB.delMatch(matches[i].id);
  }
